 ...
 * 
 * @param {Object} duLieu - Chứa user_id, task_id (có thể null), message, notification_type
...
async function taoThongBao({ user_id, task_id, message, notification_type }) {
  const ketQua = await db.truyVan(
    `INSERT INTO Notifications (user_id, task_id, message, notification_type)
     VALUES ($1, $2, $3, $4)
     RETURNING *`,
    [user_id, task_id || null, message, notification_type || "general"]
  );
...
/**
 * Xoá toàn bộ thông báo của một người dùng
 *
 * @param {number} maNguoiDung - ID người dùng
 * @returns {Promise<number>} Số lượng thông báo đã xoá
 */

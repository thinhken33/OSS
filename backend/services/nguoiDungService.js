...
const AVATAR_GIOI_HAN_BYTE = AVATAR_GIOI_HAN_MB * 1024 * 1024;
 ...
 * @returns {Promise<Object>}
 */
async function capNhatNguoiDung(maNguoiDung, duLieu) {
  const { full_name, bio } = duLieu;
  if (!full_name || !full_name.trim()) {
    throw taoLoi("Họ và tên không được để trống.", 400);
  }

  const nguoiDungHienTai = await nguoiDungModel.layNguoiDungTheoId(maNguoiDung);
  if (!nguoiDungHienTai) {
    throw taoLoi("Không tìm thấy người dùng để cập nhật.", 404);
  }

  const coGuiAvatar = Object.prototype.hasOwnProperty.call(duLieu, "avatar_url");
  const avatar_url = coGuiAvatar ? duLieu.avatar_url : nguoiDungHienTai.avatar_url;
 ...
 * 
 * @param {number} maNguoiDung - ID người dùng
 * @param {string} avatarUrl - URL avatar (base64 hoặc link ảnh)
 * @returns {Promise<Object>} Thông tin người dùng sau cập nhật
 */
async function capNhatAvatar(maNguoiDung, avatarUrl) {
  if (!avatarUrl || !avatarUrl.trim()) {
    throw taoLoi("Avatar không được để trống.", 400);
  }
  ...
  // Giới hạn kích thước ảnh thực tế
  if (avatarUrl.startsWith("data:")) {
    const ketQua = avatarUrl.match(/^data:.*;base64,(.*)$/);
    if (!ketQua || !ketQua[1]) {
      throw taoLoi("Dữ liệu ảnh không hợp lệ.", 400);
    }

    const kichThuocAnh = Buffer.from(ketQua[1], "base64").length;
    if (kichThuocAnh > AVATAR_GIOI_HAN_BYTE) {
      throw taoLoi(`Kích thước ảnh quá lớn (tối đa ${AVATAR_GIOI_HAN_MB}MB).`, 400);
    }
  }

  const nguoiDungCapNhat = await nguoiDungModel.capNhatAvatar(maNguoiDung, avatarUrl);
  return nguoiDungCapNhat;
}

/**
 * Xoá avatar của người dùng
 * 
 * @param {number} maNguoiDung - ID người dùng
 * @returns {Promise<Object>} Thông tin người dùng sau xoá avatar
 */
async function xoaAvatar(maNguoiDung) {
  const nguoiDung = await nguoiDungModel.layNguoiDungTheoId(maNguoiDung);
  if (!nguoiDung) {
    throw taoLoi("Không tìm thấy người dùng.", 404);
  }

  const nguoiDungCapNhat = await nguoiDungModel.capNhatAvatar(maNguoiDung, null);
  return nguoiDungCapNhat;
}

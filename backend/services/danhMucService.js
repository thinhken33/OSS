...
function chuanHoaIcon(icon) {
  if (icon === undefined || icon === null) return null;
  if (typeof icon !== "string") {
    throw taoLoi("Icon danh mục không hợp lệ.", 400);
  }
  const iconDaTrim = icon.trim();
  if (!iconDaTrim) return null;
  if (iconDaTrim.length > 16) {
    throw taoLoi("Icon danh mục quá dài.", 400);
  }
  return iconDaTrim;
}
 ...
 * @returns {Promise<Object>}
 */
async function taoDanhMuc(maNguoiDung, { name, icon }) {
 ...
 * @returns {Promise<Object>}
 */
async function capNhatDanhMuc(maDanhMuc, maNguoiDung, { name, icon }) {
  if (!name || !name.trim()) {
    throw taoLoi("Tên danh mục không được để trống.", 400);
  }
  ...
  return await danhMucModel.capNhatDanhMuc(maDanhMuc, {
    name: name.trim(),
    icon: chuanHoaIcon(icon),
  });
}
...
async function xoaDanhMuc(maDanhMuc, maNguoiDung) {
  ...
  // Không xoá task: đưa category_id của task trong danh mục về null
  await danhMucModel.boGanDanhMucKhoiCongViec(maDanhMuc);

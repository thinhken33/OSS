...
const { taoAccessToken } = require("../utils/authToken");
...
async function dangNhap(req, res) {
  try {
    ...
    if (!accessToken) {
      const loi = new Error("Khong tao duoc token phien dang nhap.");
      loi.statusCode = 500;
      throw loi;
    }
...
/**
 * Cập nhật avatar (ảnh đại diện) của người dùng
 * Nhận dữ liệu hình ảnh dưới dạng base64 hoặc URL
 * 
 * @param {Object} req - Đối tượng HTTP Request, chứa avatar_url trong req.body
 * @param {Object} res - Đối tượng HTTP Response
 */
async function capNhatAvatar(req, res) {
  try {
    const { avatar_url } = req.body;
    const nguoiDung = await nguoiDungService.capNhatAvatar(parseInt(req.params.id), avatar_url);
    res.json({
...
/**
 * Xoá avatar (ảnh đại diện) của người dùng
 * 
 * @param {Object} req - Đối tượng HTTP Request
 * @param {Object} res - Đối tượng HTTP Response
 */
async function xoaAvatar(req, res) {
  try {
    const nguoiDung = await nguoiDungService.xoaAvatar(parseInt(req.params.id));
    res.json({
      message: "Đã xoá avatar thành công.",
      nguoiDung,
    });
  } catch (loi) {
    const maLoi = loi.statusCode || 500;

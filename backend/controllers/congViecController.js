...
async function layCongViecTheoId(req, res) {
  try {
    const maCongViec = Number.parseInt(req.params.id, 10);
    if (!Number.isInteger(maCongViec) || maCongViec <= 0) {
      return res.status(400).json({ message: "ID cong viec khong hop le." });
    }

    const congViec = await congViecService.layCongViecTheoId(maCongViec);
    const userDangNhap = req.nguoiDungDangNhap;
    if (!userDangNhap) {
      return res.status(401).json({ message: "Vui long dang nhap de tiep tuc." });
    }

    const laChuSoHuu = userDangNhap.user_id === congViec.user_id;
    const laAdmin = userDangNhap.role === "admin";
    if (!laChuSoHuu && !laAdmin) {
      return res.status(403).json({ message: "Ban khong co quyen xem cong viec nay." });
    }

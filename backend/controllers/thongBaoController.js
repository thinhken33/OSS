...
async function xoaTatCaThongBao(req, res) {
  try {
    const maNguoiDung = parseInt(req.params.userId);
    const soLuongDaXoa = await thongBaoService.xoaTatCaThongBao(maNguoiDung);
    res.json({
      message: "Da xoa tat ca thong bao.",
      soLuong: soLuongDaXoa,
    });
  } catch (loi) {
    res.status(500).json({ message: loi.message || "Khong xoa duoc tat ca thong bao." });
  }
}
...
async function taoNhacViec(req, res) {
  try {
    ...
    res.json({
      message: `Da tao ${danhSachThongBao.length} thong bao he thong.`,
      danhSachThongBao,
    });
  } catch (loi) {
    res.status(500).json({ message: loi.message });

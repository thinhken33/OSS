 ...
 */
async function layDanhMucTheoId(maDanhMuc, maNguoiDung) {
  const danhMuc = await danhMucModel.layDanhMucTheoId(maDanhMuc);
  if (!danhMuc) {
    throw taoLoi("Không tìm thấy danh mục.", 404);
  }
  if (maNguoiDung && danhMuc.user_id !== maNguoiDung) {
    throw taoLoi("Bạn không có quyền xem danh mục này.", 403);
  }
  return danhMuc;
}
...
async function taoDanhMuc(maNguoiDung, { name }) {
  ...
  const danhMucTonTai = await danhMucModel.layDanhMucTheoTen(
    maNguoiDung,
    name.trim(),
  );
  if (danhMucTonTai) {

 ...
 */
async function dangKyNguoiDung({
  full_name,
  email,
  password,
  avatar_url,
  ...
  const nguoiDungTonTai = await nguoiDungModel.layNguoiDungTheoEmail(
    email.trim(),
  );
  if (nguoiDungTonTai) {
  ...
  const token = jwt.sign(
    { user_id: nguoiDungMoi.user_id, role: nguoiDungMoi.role },
    JWT_SECRET,
    { expiresIn: "7d" },
  );

  return { nguoiDung: nguoiDungMoi, token };
}
...
async function dangNhap({ email, password }) {
  ...
  const nguoiDung = await nguoiDungModel.layNguoiDungTheoEmail(
  ...
  const token = jwt.sign(
    { user_id: nguoiDung.user_id, role: nguoiDung.role },
    JWT_SECRET,
    { expiresIn: "7d" },
  );

  return { nguoiDung: nguoiDungKhongMatKhau, token };
}
...
async function doiMatKhau(maNguoiDung, { current_password, new_password }) {
  ...
  const nguoiDungDayDu = await nguoiDungModel.layNguoiDungTheoEmail(
    nguoiDung.email,
  );
  const matKhauKhop = await bcrypt.compare(
    current_password,
    nguoiDungDayDu.password_hash,
  );
  if (!matKhauKhop) {
 ...
 *
 * @param {number} maNguoiDung - ID người dùng
 * @param {string} avatarUrl - Đường dẫn file avatar
 * @returns {Promise<Object>}
 */
async function capNhatAvatar(maNguoiDung, avatarUrl) {
  const nguoiDung = await nguoiDungModel.capNhatAvatar(maNguoiDung, avatarUrl);
  if (!nguoiDung) {
    throw taoLoi("Không tìm thấy người dùng.", 404);
  }
  return nguoiDung;

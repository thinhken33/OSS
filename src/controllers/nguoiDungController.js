...
async function layTatCaNguoiDung(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ message: loi.message || "Không lấy được danh sách người dùng." });
  }
}
...
async function layNguoiDungTheoId(req, res) {
  try {
    const nguoiDung = await nguoiDungService.layNguoiDungTheoId(
...
async function dangKy(req, res) {
  try {
    const ketQua = await nguoiDungService.dangKyNguoiDung(req.body);
    res.status(201).json({
...
async function dangNhap(req, res) {
  try {
    const ketQua = await nguoiDungService.dangNhap(req.body);
    res.json({
...
async function doiMatKhau(req, res) {
  try {
    const ketQua = await nguoiDungService.doiMatKhau(
      parseInt(req.params.id),
      req.body,
    );
    res.json(ketQua);
...
async function datTrangThaiKhoa(req, res) {
  try {
    const is_locked = req.body.is_locked === true || req.body.is_locked === "true";
    const nguoiDung = await nguoiDungService.datTrangThaiKhoa(
      parseInt(req.params.id),
      is_locked,
    );
    res.json({
...
async function xoaNguoiDung(req, res) {
  try {
    const nguoiDung = await nguoiDungService.xoaNguoiDung(
      parseInt(req.params.id),
    );
    res.json({
 ...
 *
 * @param {Object} req - Đối tượng HTTP Request (chứa file từ multer)
 * @param {Object} res - Đối tượng HTTP Response
 */
async function taiLenAvatar(req, res) {
  try {
    if (!req.file) {
      return res.status(400).json({ message: "Vui lòng chọn file ảnh." });
    }
    const userId = parseInt(req.params.id);
    const userCu = await nguoiDungService.layNguoiDungTheoId(userId);
    const avatarCu = userCu?.avatar_url;

    const avatarUrl = "/uploads/avatars/" + req.file.filename;
    const nguoiDung = await nguoiDungService.capNhatAvatar(userId, avatarUrl);

    // Xóa file ảnh cũ nếu tồn tại
    if (avatarCu && avatarCu.startsWith("/uploads/avatars/")) {
      const oldPath = path.join(__dirname, "../../public", avatarCu);
...
/**
 * Xóa ảnh đại diện (đặt lại thành null)
 */
async function xoaAvatar(req, res) {
  try {
    const userId = parseInt(req.params.id);
    const userCu = await nguoiDungService.layNguoiDungTheoId(userId);
    const avatarCu = userCu?.avatar_url;

    const nguoiDung = await nguoiDungService.capNhatAvatar(userId, null);
    ...
    res.json({ message: "Đã xóa ảnh đại diện.", nguoiDung });
  } catch (loi) {
    const maLoi = loi.statusCode || 500;
    res.status(maLoi).json({ message: loi.message });
  }

...
async function layDanhMuc(req, res) {
  try {
    ...
    const danhSachDanhMuc =
      await danhMucService.layDanhMucTheoNguoiDung(maNguoiDung);
    res.json(danhSachDanhMuc);
  } catch (loi) {
    res
      .status(500)
      .json({ message: loi.message || "Không lấy được danh mục." });
  }
}
...
async function layDanhMucTheoId(req, res) {
  try {
    const maNguoiDung = parseInt(req.user.user_id);
    const danhMuc = await danhMucService.layDanhMucTheoId(
...
async function capNhatDanhMuc(req, res) {
  try {
    const maNguoiDung = parseInt(req.user.user_id);
    const danhMuc = await danhMucService.capNhatDanhMuc(
      parseInt(req.params.id),
      maNguoiDung,
      req.body,
    );
...
async function xoaDanhMuc(req, res) {
  try {
    const maNguoiDung = parseInt(req.user.user_id);
    const danhMuc = await danhMucService.xoaDanhMuc(
      parseInt(req.params.id),
      maNguoiDung,
    );
    res.json({

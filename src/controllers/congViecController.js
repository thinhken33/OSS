...
async function layCongViec(req, res) {
  try {
    const maNguoiDung = parseInt(req.user.user_id);
    const danhSachCongViec =
      await congViecService.layCongViecTheoNguoiDung(maNguoiDung);
    res.json(danhSachCongViec);
  } catch (loi) {
    res
      .status(500)
      .json({ message: loi.message || "Không lấy được danh sách công việc." });
  }
}
...
async function layCongViecTheoId(req, res) {
  try {
    const maNguoiDung = parseInt(req.user.user_id);
    const congViec = await congViecService.layCongViecTheoId(
...
async function capNhatCongViec(req, res) {
  try {
    const maNguoiDung = parseInt(req.user.user_id);
    const congViec = await congViecService.capNhatCongViec(
      parseInt(req.params.id),
      maNguoiDung,
      req.body,
    );
...
async function capNhatTrangThaiCongViec(req, res) {
  try {
    ...
    const congViec = await congViecService.capNhatTrangThaiCongViec(
      parseInt(req.params.id),
      maNguoiDung,
      status,
    );
...
async function xoaCongViec(req, res) {
  try {
    const maNguoiDung = parseInt(req.user.user_id);
    const congViec = await congViecService.xoaCongViec(
      parseInt(req.params.id),
      maNguoiDung,
    );
    res.json({
...
async function timKiemCongViec(req, res) {
  try {
    ...
    const danhSachCongViec = await congViecService.timKiemCongViec(
      maNguoiDung,
      boLoc,
    );
    res.json(danhSachCongViec);
...
async function layThongKeCongViec(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ message: loi.message || "Không lấy được thống kê." });
  }
}

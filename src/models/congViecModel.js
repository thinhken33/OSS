...
async function taoCongViec({
  ...
  const ketQua = await db.truyVan(
    `INSERT INTO Tasks (user_id, category_id, title, description, start_date, due_date, priority, status, notification_times)
    ...
    [
      ...
      priority || "medium",
      status || "pending",
      notification_times ? JSON.stringify(notification_times) : "[]",
    ],
  );
...
async function timKiemCongViec(
  ...
  if (tuKhoa) {
    cauTruyVan += ` AND (LOWER(t.title) LIKE LOWER($${thamSo.length + 1}) OR LOWER(t.description) LIKE LOWER($${thamSo.length + 1}))`;
    thamSo.push(`%${tuKhoa}%`);
  }

  if (trangThai) {
    cauTruyVan += ` AND t.status = $${thamSo.length + 1}`;
    thamSo.push(trangThai);
  }

  if (mucUuTien) {
    cauTruyVan += ` AND t.priority = $${thamSo.length + 1}`;
    thamSo.push(mucUuTien);
  }

  if (maDanhMuc) {
    cauTruyVan += ` AND t.category_id = $${thamSo.length + 1}`;
    thamSo.push(maDanhMuc);
  }

  if (hanHoanThanh) {
    cauTruyVan += ` AND DATE(t.due_date) = $${thamSo.length + 1}`;
    ...
    `SELECT * FROM Tasks
     WHERE user_id = $1
       ...
       AND (
         ...
         OR EXISTS (
            SELECT 1 FROM jsonb_array_elements_text(COALESCE(notification_times, '[]'::jsonb)) AS t
            WHERE t::timestamp <= NOW()
         )
         OR (
            (notification_times IS NULL OR jsonb_array_length(COALESCE(notification_times, '[]'::jsonb)) = 0)
            AND due_date IS NOT NULL AND due_date <= NOW() + INTERVAL '1 day'
         )
       )
